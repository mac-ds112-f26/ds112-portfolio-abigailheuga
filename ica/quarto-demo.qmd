...
> file is stored locally, easier to read, not editable

Toggling between the .qmd and .html files, explain the purpose of the following features in the .qmd file:        

`*`
> makes a word italisized (when surrounding)

`**`
> makes a word bold (when surrounding)

`#`
> makes a comment

`-`
> makes a bullet point/bulleted list

`\`
> represents special charactiers insdie strings (R code), i.e. forces hard line breaks

`![](url)`
> imports a link to a website
...
> appears as text in .qmd and .html, not code
...
> R code is in a separate box that is clearly distinguishable from the regular text, output is also returned separately from the R code and normal text
